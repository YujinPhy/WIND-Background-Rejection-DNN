...
def get_channels(x, y, z, pe_times, charges, width=142, side_height=45, cap_res=23):
    # 1. 예외 처리: 데이터가 없는 경우 빈 배열 반환
    if len(pe_times) == 0:
        total_height = side_height + (2 * cap_res)
        return np.zeros((2, total_height, width), dtype=np.float32)

    z_min, z_max, r_max = -3000.0, 3000.0, 3000.0
    total_height = side_height + (2 * cap_res)
    
    channel_num = 2
    chw = np.zeros((channel_num, total_height, width), dtype=np.float32)
    ...
    for i in range(len(x)):
        ...
        zi = np.clip(zi, 0, total_height - 1) # 안전장치

        # Charge 누적
        chw[0, zi, pi] += charges[i]

        # Time (가장 빠른 시간 기록)
        relative_t = pe_times[i] - t0
        if relative_t < chw[1, zi, pi]:
            chw[1, zi, pi] = relative_t

    # --- 중요: 후처리 및 정규화 ---
    
    # 1. Charge 정규화 (Max scaling)
    ch_max = np.max(chw[0])
    if ch_max > 0:
        chw[0] = chw[0] / ch_max

    # 2. Time 정규화 (반전 정규화 추천)
    # 히트가 있는 마스크 생성
    hit_mask = chw[1, :, :] < 999999.0
    
    # 논문 스타일 윈도우 (예: 270ns)를 써도 되고, 현재 이벤트의 max를 써도 됩니다.
    # 여기서는 현재 이벤트 내부의 max로 정규화합니다.
    valid_times = chw[1, hit_mask]
    if len(valid_times) > 0:
        t_max = np.max(valid_times)
        if t_max > 0:
            # 반전 정규화: 첫 히트(0ns) -> 1.0, 마지막 히트(t_max) -> 0.001 (또는 0.1)
            # 신호가 없는 곳은 0.0을 유지하도록 함
            chw[1, hit_mask] = 1.0 - (chw[1, hit_mask] / (t_max + 1e-9))
        else:
